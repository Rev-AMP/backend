...
def create_random_user(db: Session) -> User:
    ...
    user_in = UserCreate(username=email, email=email, password=password, type="superuser")
...
def authentication_token_from_email(*, client: TestClient, email: str, db: Session) -> Dict[str, str]:
    ...
    if not user:
        user_in_create = UserCreate(username=email, email=email, password=password, type="student")
